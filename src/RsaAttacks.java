...
public class RsaAttacks {
    ...
    public static ArrayList<BigInteger> China(ArrayList<BigInteger> Ci, ArrayList<BigInteger> Ni) {
        ...
        BigInteger N = new BigInteger("1", 16);
    ...
    public static String NumbRoot(BigInteger A) {
        ...
        for (int i = 1; i < delimnum.size(); i++) {
            ...
            while (!signal) {
                ...
                x1 = x;
                for (BigInteger bigInteger : form) {
                    sum1 = sum1.add(x1.multiply(bigInteger));
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("N = " + N.toString(16));
        long l;
        l = (int) Math.pow(2, 10); // regularization l=20
        ...
        for (int i = 0; i < X.size(); i++) {
            ...
            for (int j = 0; j < X.size(); j++) {
                if (Cs.equals(X.get(j)) && !Objects.equals(X.get(j), BigInteger.ONE)) {
                    flag = j;
                    break;
                }
            }
        ...
        if (flag == -1) System.out.println("Didn`t find any");
            ...
            BigInteger M2Big = new BigInteger(String.valueOf(flag+1), 10);
            BigInteger M1Big = new BigInteger(String.valueOf(flagX), 10);

            //Verify
            System.out.println("Expected -- M1 = " + M1Big.toString(16));
            System.out.println("Expected -- M2 = " + M2Big.toString(16));
            System.out.println("M1+M2 "+ (M1Big.multiply(M2Big)).toString(16));
            System.out.println("new (M1*M2)^e "+ (M1Big.multiply(M2Big)).modPow(e, N).toString(16));
            System.out.println("Real C = " + C.toString(16));
